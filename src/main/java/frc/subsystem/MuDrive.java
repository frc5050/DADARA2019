...
public class MuDrive extends Subsystem {
    ...
    private static MuDrive instance;
    ...
    public static MuDrive getInstance() {
        if (instance == null) {
            instance = new MuDrive();
        }
        return instance;
    }
    ...
    public synchronized void updatePathFollower() {
        if (state == DriveState.PATH_FOLLOWING) {
            ...
            if (!this.isDone()) {
                ...
                double rightVelocity = leftTrajectory.get(lastTrajectoryValue).velocity;
